...
Operates on all text: inline code, code chunks and regular text.

.Large[.Large `.Large[]` 144%]

.large[.large `.large[]` 120%]

.small[.small shrinks to 90% works on `inline code` as well as code chunks
...
Link behaves strangely.

---

## Central limit theorem $\LaTeX$
...
.footnote[The inline MathJax is for some reason intrinsically linked to inline code, so if you have a background colour for inline code, it will be the same for the inline MathJax.]
